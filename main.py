...
def main():
    ...
    if len(sys.argv) != 8:
        ...
        return -1
    # Assign args
    train_input, test_input, index_to_word, index_to_tag, hmm_prior, hmm_emit, hmm_trans = sys.argv[1:]
    #
    # TRAIN
    #
    ...
    # TEST
    #
    for test_train in [train_input, test_input]:
        # Get the parsed data from the input files
        sentences, tags, word_dict, tag_dict = hmm.parse_tags_words_to_index(index_to_word, index_to_tag, test_train)
        # Do the forward computation for alpha and beta
